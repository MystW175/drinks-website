"use client";

import Image from "next/image";
import PriceTag from "@/components/priceTag";
import Link from "next/link";

import { CartItemWithProducts } from "@/lib/db/cart";
import IncrementButton from "@/components/IncrementButton";
import { useTransition } from "react";

interface CartEntryProps {
    cartItem: CartItemWithProducts;
    setProductQuantity: (productId: string, quantity: number) => Promise<void>;
}
export default function CartEntry({ cartItem: { product, quantity }, setProductQuantity, }: CartEntryProps) {
    const [isPending, startTransition] = useTransition();
    
    function decrementNumber() {
        if(!isPending){
            var newQuantity = quantity - 1;
            startTransition(async () => {
                await setProductQuantity(product.id, newQuantity)
            })
        }
    }
    
    function incrementNumber() {
        if(!isPending){
            if(quantity<99){
                var newQuantity = quantity + 1;
                startTransition(async () => {
                    await setProductQuantity(product.id, newQuantity)
                })
            }
        }
    }

    function ItemDelete(){
        startTransition(async () => {
            await setProductQuantity(product.id, 0)
        })
    }

    return (
        <div className="flex gap-6 rounded-lg items-center w-full sm:w-max">
            <div className="indicator">
                <button onClick={ItemDelete} className="indicator-item badge btn-outline hover:btn-error bg-white p-1">
                <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12.8536 2.85355C13.0488 2.65829 13.0488 2.34171 12.8536 2.14645C12.6583 1.95118 12.3417 1.95118 12.1464 2.14645L7.5 6.79289L2.85355 2.14645C2.65829 1.95118 2.34171 1.95118 2.14645 2.14645C1.95118 2.34171 1.95118 2.65829 2.14645 2.85355L6.79289 7.5L2.14645 12.1464C1.95118 12.3417 1.95118 12.6583 2.14645 12.8536C2.34171 13.0488 2.65829 13.0488 2.85355 12.8536L7.5 8.20711L12.1464 12.8536C12.3417 13.0488 12.6583 13.0488 12.8536 12.8536C13.0488 12.6583 13.0488 12.3417 12.8536 12.1464L8.20711 7.5L12.8536 2.85355Z" fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"></path></svg>
                </button> 
                <Link className="max-w-[14rem] hover:scale-[.98] transition-all" href={'/products/' + product.id}>
                    <Image src={product.imageUrl} alt={product.name} width={200} height={200} className="rounded-lg object-cover w-full" priority></Image>
                </Link>
            </div>
            <div className="w-[55%] min-w-[11rem] sm:w-64 flex-1 flex flex-col gap-2">
                <p className="font-bold sm:text-lg">{product.name}</p>
                <div className="flex items-center gap-3">
                    <PriceTag className="border-none text-sm p-0" price={product.price} />
                </div>
                <div className="flex items-center gap-3">

                    <div className="flex justify-center w-fit items-center gap-1">
                        <button className="btn btn-outline btn-xs btn-square" onClick={decrementNumber}>
                            <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M2.25 7.5C2.25 7.22386 2.47386 7 2.75 7H12.25C12.5261 7 12.75 7.22386 12.75 7.5C12.75 7.77614 12.5261 8 12.25 8H2.75C2.47386 8 2.25 7.77614 2.25 7.5Z" fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"></path></svg>
                        </button>
                        <p>{quantity}</p>
                        <button className="btn btn-outline btn-xs btn-square" onClick={incrementNumber}>
                            <svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8 2.75C8 2.47386 7.77614 2.25 7.5 2.25C7.22386 2.25 7 2.47386 7 2.75V7H2.75C2.47386 7 2.25 7.22386 2.25 7.5C2.25 7.77614 2.47386 8 2.75 8H7V12.25C7 12.5261 7.22386 12.75 7.5 12.75C7.77614 12.75 8 12.5261 8 12.25V8H12.25C12.5261 8 12.75 7.77614 12.75 7.5C12.75 7.22386 12.5261 7 12.25 7H8V2.75Z" fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"></path></svg>
                        </button>
                         {isPending && <span className="loading loading-spinner loading-sm"/>}
                    </div>
                </div>
                <div className="flex items-center gap-3">
                    <p className="text-sm sm:text-base">Subtotal: </p>
                    <PriceTag className="px-0 border-none sm:text-base" price={quantity * product.price} />
                </div>
            </div>
        </div>
    );
}